...
A review-pending WHO/UNFPA public-source corpus is indexed in Foundry File Search and a `mama-link-knowledge` agent was verified against it. It is a retrieval prototype, not an approved clinical corpus. Application Insights is connected to the Foundry project with automatic HTTP/Azure SDK instrumentation disabled; the app emits aggregate metrics and metadata-only agent spans. On 2026-09-25 the final selected Foundry evaluation runs passed all 24 synthetic cases across the seven agents. Clinical approval, held-out/red-team evaluation and authenticated deployment remain pending. The patient workflow remains deterministic. See [the project assessment](docs/project-assessment.md), [Foundry setup](docs/foundry-setup.md), and [cloud evaluation evidence](docs/foundry-evaluation-latest.json).
